...
> **Last touched:** 2026-05-19 (**Round 5 continuation + 5 ship branches.** 10-agent continuation of 5/18 across date roll. Every agent in `git worktree add` isolation (0 races). 5 new ship branches off master `86ca1ed`: `31e0fc9` pfizer-name-fix (new `normalize_name_legal_suffixes_only` helper + AST CI guard test + back-fill script preview-by-default; 189/189 tests; **preview shows 32,204 + 36,627 rows actionable — 1.7x design-doc estimate**), `7cfe24e` def14a-suffix-fix (new `director_name_sanitizer.py` strips 5 pattern classes; 87 of 23,956 director rows / 23 masters affected; 97 tests), `72394f3` fix-rematch-test (tighter `_skip_no_matchable_pool` gate runs script's NAME_AGGRESSIVE SQL across 4 sources; closes Open Problem), `510eeaa` sec10k-tooling-on-master (13 files / 5,264 LOC bit-identical to q16-19 — actual script name `identify_recent_10k.py`; **unblocks Week 3 download**), `acad2db` drop-director-overfetch DEFERRED (doc-only; premise wrong — SQL/Python parity holds on boardcard branch not master). **Codex Round 5 re-crosscheck flagged 4 real bugs deferred to Round 6**: backfix_def14a rollback loses n_applied; live def14a parser doesn't pass filer_company; backfix JOIN assumes cik types; normalize_name_legal_suffixes_only strips anywhere. **No new file structure on master** (all new files live on ship branches); category sections unchanged. **Vault**: 5 scratch reports (CWA design, branch audit, MV investigation, test brittleness, MEMORY trim). 1 Open Problem RESOLVED (Rematch Test Drained Pool); 2 advanced (Pfizer fix shipped + migration ready; mv_target_data_sources H1 confirmed). Napkin: 3+ new patterns. MEMORY.md trimmed 72KB→24KB. **Week 3 opens today**. Atlas unchanged at 236 tasks. Memory: `memory/session_2026_05_19_round5_10_agent_continuation.md`. Vault: `Work Log/2026-05-19 - Round 5 10-Agent Continuation + 5 Ship Branches.md`. No git commits to master.)
>
> **Earlier 2026-05-18:** (**18-agent parallel sweep, 7 ship branches.** 3 rounds closing Week 2 of the 9-week launch roadmap. **7 ship branches** pushed off master `86ca1ed`: `f1da715` gower-mv-tail-check, `b8509d9` fec-nameerror-fix (cherry-pick of d850d54, active production bug closure), `a45ccc8` boardcard-filter-fix (Boeing 165 + Walmart 180 garbage interlocks dropped via filter at response boundary), `c83a5b7` fix-test-collection-error (backend 1,580+1err -> 1,596+0err via audit_boardcard_quality.py cherry-pick), `d5ca74d` doc-count-refresh, `4728abc` 10k-nonaggressive-fallback (new `match_extracted_entities_v2.py` standalone), `b5c8764` 10k-stop-list-iter-3 (off q16-19, +27 entries with new `_STOP_PATTERN` collection). **No new file structure on master itself** (every new file lives on ship branches); DOCUMENT_INDEX category sections unchanged. **Vault**: 40 Open Problems closed + 1 reopened (`mv_target_data_sources source_count undercount` — 5/12 fix didn't take) + 2 new opened: `Pfizer Master Canonical Name Corruption` (root cause REPRODUCED bit-exactly at `load_mergent_universal.py:154-162` `str.replace()` substring bug, 19,140 master_employers rows affected, 99.9% mergent-origin, introduced 2026-05-03 fe0667b) + `Unmerged Ship-Branch Backlog Concealing Fixes` (HIGH severity). **24Q coverage 17/6/3/0 already exceeds July 5 target 15/6/3/0**. Today is end of Week 2. New session memory `memory/session_2026_05_18_18_agent_sweep_7_ship_branches.md` + vault `Work Log/2026-05-18 - 18-Agent Parallel Sweep + 7 Ship Branches.md`. 10 scratch reports incl master synthesis at `docs/scratch/2026_05_18_session_synthesis.md`. Atlas unchanged at 236 tasks (session was bug fixes + process, not new roadmap items). No git commits to master. **Round 4 (codex follow-up)**: dispatched after first /wrapup's codex crosscheck flagged 3 BUGs across `4728abc` (10k-nonaggressive-fallback) and `a45ccc8` (boardcard-filter-fix). 2 parallel agents (both via `git worktree add`, zero races) fixed them as `7223116` (Assoc expansion + end-anchored suffix on 10k branch) and `419606b` (SQL parity + bonus 46-row punctuation delta on board branch). Codex re-crosscheck found 1 P2 residual (numeric first-token RTRIM missing); fixed inline as `75a015e`. Branch tip now `7223116` and `75a015e`. **All 3 Codex BUGs + 1 P2 closed**; user-visible bug on master 4238837 ("8 directors" with empty list on BoardCard) closed before reaching production.)
>
> **Earlier 2026-05-12:** (**11 ship branches parallel sweep + Gower restore.** Two consecutive 5-6 agent rounds + ops, 13 ship branches incl 2 atlas. AM: mv_target_data_sources undercount fix + EmptyStateCard component + FEC+SEC XBRL future-date cleanup (silent NameError fix landed on `ship/2026-05-12-future-date-cleanup` d850d54 — today's `b8509d9` cherry-picks it to master because the branch was never merged) + small-ops sweep + parse_verdict_obj helper + atlas +9. PM: 10-K canonical_name_aggressive column + CBA 89.9% match rate + Stability Pillar resolved + compute_research_enhancements 58% miss rate fix + broken date_query refs + 138 terminated fnums backfill. Atlas: 215->236 tasks. New session memory `memory/session_2026_05_12_11_ship_branches_parallel_sweep.md`. NEW Open Problem `Gower Compute Drops Scorecard MVs` HIGH — closed today via `ship/2026-05-18-gower-mv-tail-check`.)
>
> **Earlier 2026-05-09:** (**Q16/17/19 chain shipped via 6-agent parallel sprint + critical MV regression discovered.** No git commits. **9 new code files** spread across the existing sections (no new categories): `scripts/etl/sec_10k/extract_relationship_entities.py` (Agent 1, ~550 lines, heuristic 10-K entity extractor), `scripts/etl/sec_10k/match_extracted_entities.py` (Agent 2, V2-cascade matcher with `NULLS NOT DISTINCT` idempotency), `scripts/research/audit_boardcard_quality.py` (Agent 5, 9-check programmatic QA), `api/routers/relationships.py` (Agent 3, 3 endpoints sharing `_fetch_relationships()`), `tests/etl/test_sec_10k_extract_entities.py` (14 tests), `tests/etl/sec_10k/test_match_extracted_entities.py` + `__init__.py` (25 tests), `tests/test_relationships_router.py` (12 tests, 6 + 6 seeded), `tests/research/test_boardcard_quality.py` (20 tests). **4 new frontend cards**: `frontend/src/features/employer-profile/{Suppliers,Customers,Distribution}Card.jsx` + shared `RelationshipBody.jsx` (Agent 4, mirrors `LobbyingCard` chrome), with 3 vitest files in `frontend/__tests__/` (16 tests). **Modified**: `api/main.py` (+1 line relationships router include), `frontend/src/features/employer-profile/EmployerProfilePage.jsx` (3 imports + 3 mounts at 273-279), `frontend/src/shared/api/profile.js` (`_useRelationships` factory + 3 named exports), `frontend/src/features/employer-profile/ComparablesCard.jsx` + `frontend/__tests__/ComparablesCard.test.jsx` (Agent 6 polish: skeleton + amber empty + retry button; 7 tests up from 5). **3 docs/scratch reports**: `sec_10k_extraction_2026_05_09.md`, `sec_10k_matching_2026_05_09.md`, `boardcard_audit_2026_05_09.md`. **DB**: `sec_10k_extracted_entities` + `sec_10k_relationship_links` tables CREATEd and populated (25 + 25 rows from sample runs). **Critical MV regression**: `mv_target_scorecard` (~5.4M expected) + `mv_employer_search` (~100K expected) both MISSING from `pg_matviews` — second occurrence for `mv_target_scorecard` (also 2026-04-30); `scripts/maintenance/check_critical_mvs.py` blocks /ship; rebuild via `py scripts/scoring/refresh_all.py --skip-gower`. **Vault NEW**: `Open Problems/Critical MVs Missing After 2026-05-08.md` (CRITICAL). **Vault MODIFIED**: 2 closed Open Problems (F7 Orphan Rate Regression high->resolved at 64.37%; Demographics API Serves Raw ACS Not V10 medium->resolved via V12 in `demographics.py:235`). New session memory at `memory/session_2026_05_09_q16_19_chain_and_mv_discovery.md` + vault `Work Log/2026-05-09 - Q16-17-19 Chain Shipped + Critical MV Discovery.md`. No DOCUMENT_INDEX category-level structural changes — all new files fit existing sections. **Codex crosscheck (during /wrapup) flagged 3 BUGs, all fixed**: `/employer/MASTER-` -> `/employers/MASTER-` URL fix across `RelationshipBody.jsx` + 2 pre-existing-broken cards (`BoardCard.jsx`, `CompetitorsCard.jsx`) + 5 test files; matcher UNIQUE constraint reshaped to `(source_entity_id, relationship_type)` with `ON CONFLICT DO UPDATE`; extractor `write_entities()` now called unconditionally for proper idempotency. 51/51 backend + 44/44 frontend tests pass post-fix. **Codex round 2 (re-run on post-fix diff)** flagged 3 more BUGs: (a) constraint migration was incomplete (`CREATE TABLE IF NOT EXISTS` skips on existing tables, leaving no `entity_type_uq` constraint to honor `ON CONFLICT ON CONSTRAINT`) -- patched with separate `ALTER TABLE ... ADD CONSTRAINT IF NOT EXISTS` block; (b) `normalize_name_aggressive` vs `canonical_name` suffix asymmetry depresses match rate -- logged as Open Problem `10-K Matcher Suffix Stripping Asymmetry.md`, deferred post-launch P1; (c) `formatFilingDate()` UTC parse displayed dates off-by-one in US time zones -- patched to local-midnight construction. 51/51 backend + 16/16 frontend cards verified.)
>
> **Earlier 2026-05-08:** (**B.3.x Option C — rule engine integrated into rematch executor + commit at `--min-score 0.92` + over-broad regex fix.** 3 commits on `ship/2026-05-03-multi-sprint-bundle` (now merged to master via PR #1 `97a4e79`). **New code file**: `scripts/analysis/build_orphan_rematch_review_xlsx.py` (490 lines, 7-sheet Excel threshold-decision workbook). **Modified**: `scripts/matching/rematch_recoverable_orphans.py` (322-line delta — wires `classify_pair_v2` into the SQL cascade as veto + upgrade + review-routing layer; 2 new CLI flags `--review-csv` / `--no-rule-engine`; CSV schema appended back-compat with `rule_engine_tier` / `rule_engine_rule` / `rule_engine_precision`); `scripts/llm_dedup/rule_engine.py` (TRAILING_TOKEN regex `\s*` → `\s+` for series/ser/fund/trust/chapter/local — was over-broadly matching "Services" / "Funded" / "Trustees" / "Chapters" tokens); `tests/test_rematch_recoverable_orphans.py` (8 new rule-engine unit tests + 1 regression guard for the over-broad fix; per-test `_skip_no_staging` decorator instead of module-level `pytestmark` so unit tests run on a fresh DB). **Rematch committed**: 1,184 net-new active rows in `unified_match_log` across two run_ids (`orphan_rematch_20260508_134421` + `orphan_rematch_20260508_140452`). **F7 orphan rate 65.18% → 64.37%** — first time below the R6 baseline of 64.7% since Splink retirement. **Tests**: 14/14 rematch + 221/221 across rule_engine + matching + rematch + contracts + nlrb test scope; no production regressions in `match_nlrb_nightly_to_masters.py` or `match_state_local_contracts_to_masters.py` which also call `classify_pair_v2`. New session memory at `memory/session_2026_05_08_b3x_option_c_rematch_committed.md` + vault `Work Log/2026-05-08 - B.3.x Orphan Rematch Committed at 0.92.md`.)
>
> **Earlier 2026-05-05:** (**Auth Week 1 + V12 Demographics Un-Silenced.** 3 commits today on `ship/2026-05-03-multi-sprint-bundle` (now 19 since master). **New code files**: `scripts/maintenance/backfill_master_naics.py` (~225 lines, Mergent + SEC SIC->NAICS NAICS backfill); `tests/test_backfill_master_naics.py` (4 tests). **Modified**: `api/config.py` (JWT_EXPIRY_HOURS 8->1 + new JWT_REFRESH_BEFORE_EXPIRY_SECONDS); `api/dependencies.py` (NEW `require_researcher`); `api/routers/auth.py` (researcher role pattern, audit-log wiring at 5 sites, lazy-import wrapper to survive formatter, `_client_ip_for_audit` IP validation, password min_length 8->12); `api/routers/demographics.py` (ZIP+4 strip + V12 WARNING log on exception); `frontend/src/shared/hooks/useAuth.js` (silent auto-refresh at ~50min mark + single-flight guard); `frontend/src/shared/stores/authStore.js` (NEW `refreshToken` action); `tests/test_auth.py` (10 new tests, 4 pw updates). **DB**: master_employers.naics 2,246,709 -> 2,310,363 (+63,654 via Mergent + SEC SIC->NAICS); auth_audit_log now actively populated. **Findings**: V12 demographics was silently dead for ~99%% of SEC masters until today's 3-bug fix; Abbott now v12_qwi GREEN (was acs_state RED). New session memory at `memory/session_2026_05_05_auth_week1_v12_unsilenced.md` + vault `Work Log/2026-05-05 - Auth Week 1 + V12 Demographics Un-Silenced.md`.)
>
> **Earlier 2026-05-04:** (**BoardCard 24Q-14 Ship + DEF14A Scale + 9-Week Launch Roadmap.** 10 commits, 7 Open Problems closed, branch `ship/2026-05-03-multi-sprint-bundle` now 15 commits since master. **New code files**: api/routers/board.py + frontend/src/features/employer-profile/BoardCard.jsx (24Q-14 BoardCard end-to-end); scripts/maintenance/check_orphan_rate.py + tests/test_check_orphan_rate.py (F7 orphan rate release gate); scripts/matching/identify_recoverable_orphans.py + tests/test_identify_recoverable_orphans.py (Week 2 prep, 15,537 candidates staged); sql/schema/auth_audit_log.sql + auth_log.py + tests/test_auth_log.py (Week 1 prep); scripts/maintenance/report_boardcard_reach.py (launch metric); tests/test_board.py + frontend/__tests__/BoardCard.test.jsx (24Q-14 tests). **First commit of pre-existing untracked file**: scripts/etl/contracts/load_ny_abo.py (with preflight + schema-drift detection added). **Modified**: api/main.py + api/data_source_catalog.py + frontend/src/features/employer-profile/EmployerProfilePage.jsx + frontend/src/shared/api/profile.js + scripts/etl/create_master_employers.sql + scripts/etl/load_def14a_directors.py + Start each AI/CLAUDE.md + RELEASE_CHECKLIST.md + config/critical_routes.txt. **Vault NEW**: ROADMAP_2026_05_04_to_2026_07_05_LAUNCH.md (9-week launch plan, source of truth for what is next; wired into /start + /wrapup + MEMORY + CLAUDE.md). **Vault MODIFIED**: CLAUDE.md (launch roadmap reference); 7 Open Problem notes (resolutions documented). **DEF14A full-batch run COMPLETE** ~2.5 hr: 7,819 filers / 23,697 directors written / 0 failures / 64%% real-world success rate where DEF14A exists. DB final: employer_directors 121 -> 23,956 (+197x); director_interlocks view 1 -> 62,899; mv_target_data_sources 5.6M -> 5.7M; master_employers 144,881 is_union backfill. New session memory at memory/session_2026_05_04_boardcard_and_is_union_fix.md + vault Work Log/2026-05-04 - BoardCard Ship + DEF14A Scale + 9-Week Launch Roadmap.md. No DOCUMENT_INDEX category-level structural changes — all new files fit existing sections (api/routers, frontend cards, scripts/maintenance, scripts/matching, sql/schema, tests).)
>
> **Earlier 2026-05-02 PM-late:** (**DeepSeek V4 A/B Test and Rule Mining** -- Costed V4-Pro 75% promo against deferred-API-spend backlog (ends 2026-05-31). Built reusable A/B harness `scripts/llm_dedup/deepseek_ab_test.py` (569 lines, NEW, untracked). Ran 200-pair A/B on V4-Flash and V4-Pro vs 39K Haiku v2.0 ground-truth. **V4-Flash: 77.5%/87.0% / $0.044 / $1,112 5M-pair extrapolation (3.1× cheaper than Haiku batch).** **V4-Pro promo: 80.5%/89.0% / $0.213 / $5,337 5M extrapolation (worse than Haiku batch).** Both below 92% confidence bar. V4-Pro fixes V4-Flash's PARENT_CHILD parse-fail bug (5/6→0/0 empty content) and returns BROKEN labels. DeepSeek prompt caching is automatic (no `cache_control` flag); cache hit 91.9%/91.4% on the 5,230-token system prompt. **Killer finding**: hand-verified Haiku ground truth on AWAKE.ORG / CRAWFORD CONTRACTING / UNIVERSAL MALL via direct DB lookups — Haiku had **hallucinated R1 EIN-conflict violations** (R1 requires BOTH records have EINs; only one EIN existed in 2 of 3 "false merge" cases). True V4-Flash false-merge rate is ~1.0% not 1.5-3.5%; in 4 of 8 disagreement cases DeepSeek was MORE correct than Haiku. Triple-convergence (Haiku+Flash+Pro all agree) = 146/200 pairs, usable cheap-ground-truth method. **Mined 39K Haiku set for rule candidates**: 4 hard rules at ≥95% concentration (12% of pairs at 99.79% accuracy); 6 strong at ≥90% (57% at 95%). Most signals already covered by H1-H16 in `rule_engine.py`. **4 new rule candidates drafted but NOT coded** (Jacob deferred to next session): (1) real-EIN-conflict guard hardening `has_ein_conflict()` to require BOTH records have non-empty EINs; (2) H17 cross_src_one_ein_same_zip → DUPLICATE/MEDIUM (catches AWAKE/CRAWFORD pattern); (3) H18 industry_mismatch hard rule (different 2-digit NAICS prefix → UNRELATED/HIGH); (4) H19 GLEIF person-name → BROKEN. Coverage math: tuned rule_engine + V4-Flash hybrid = ~$400/5M (vs V4-Flash alone $1,112 vs Haiku batch $3,500-4,200). **Recommendation**: skip V4 for dedup sweep (keep Haiku or build rule-engine + V4-Flash hybrid); use V4-Flash for cheap deferred items (10-K employee extraction $48 vs $140 Gemini, union scraper distillation $1.50 vs $30 Sonnet, dossier search $15/100 vs $100 Gemini). DeepSeek `.env` line `DeepSeek API=sk-...` has space in var name — trips python-dotenv; built regex fallback. OpenAI SDK 2.20.0 fully compatible with `base_url="https://api.deepseek.com"`. V4 thinking-mode reasoning tokens consumed initial 300-token budget — bumped max_tokens to 3000. **Earlier "CSV verdict bug" audit chip filed at start of session was duplicate of the same-day fix** (CSV is now correct on disk). Total API spend this session $0.26. No DB changes. No code edits to existing files. No git commits. New session memory at `memory/session_2026_05_02_deepseek_v4_ab_test.md` + vault `Work Log/2026-05-02 - DeepSeek V4 A_B Test and Rule Mining.md`.)
>
> **2026-05-03:** **Omnibus Day -- /ship + REG-2 + FEC 3-cycle + SEC Ex21 + REG-4 + 24Q-12 skeleton.** Two commits on `ship/2026-05-03-multi-sprint-bundle` (`fe0667b` 297-file AM bundle + `84db90a` 8-file PM follow-up). Master push denied; PR pending Jacob. **Data**: NLRB predicted_win_pct 56K -> 529K (100% mergent); FEC indiv 22+24+26 = 144,678,426 rows / 23,533 master links / 23,445 distinct masters (Q24 Political Missing -> Strong); SEC Ex21 production --all = 77,199 new edges across 1,637 parents (closes REG-6 / 24Q-17, 0 failures); F7 OSHA + WHD rematch 19,922 H/M new matches, orphan rate 68.1% -> 65.7% (-2.36pp). **REG-2 backup automation fixed** (dropped -RunLevel Highest from PS1, full python launcher path). **Anger + weighted "regression" diagnosed as stale tests** (R7-17 + R7-18 mismatches; both fixed). **NLRB endpoint shape**: added flat `summary` alias to `/api/employers/{id}/nlrb` (R7-12 contract). **24Q-12 DEF14A skeleton**: schema + loader + parser stub mirroring Ex21 client. **FEC multi-cycle wrapper** (`scripts/etl/load_fec_extra_cycles.py`) for ON-CONFLICT idempotent appends. MV refresh chain rebuilt in 8 min. **Open Problems**: closed 3 (FEC indiv24, NLRB Predicted Win Pct, Dead Code UnifiedScorecardPage); opened 1 (FEC Garbage Future Dates). Backend 1,435 / 6 skipped; frontend 326/326. New session memory `memory/session_2026_05_03_omnibus_day.md` + vault `Work Log/2026-05-03 - Omnibus Day Ship FEC SEC Ex21 REG-2 REG-4 24Q-12.md`.
>
...
| `scripts/etl/load_def14a_directors.py` | NEW 2026-05-03 / SKELETON | 24Q-12 board-of-directors loader (DEF14A proxy parser). EDGAR client + rate limiter + DB writer + CLI mirroring `load_sec_exhibit21.py`. `parse_directors()` is a stub; production strategies (table-summary, bio-section, comp-table, prose) documented in docstring. Schema in `sql/schema/employer_directors_migration.sql` (employer_directors + load_def14a_progress + director_interlocks view). |
| `scripts/etl/load_fec_extra_cycles.py` | NEW 2026-05-03 / ACTIVE | FEC multi-cycle append wrapper. `load_fec.py` is single-cycle and DROPs+CREATEs; this wrapper imports the helpers and runs only indiv ingestion against additional cycle files (indiv22, indiv26), relying on `ON CONFLICT (sub_id) DO NOTHING` for idempotence. Used 2026-05-03 to load all 3 cycles (cycle 22 63.9M + cycle 24 58.2M + cycle 26 22.6M = 144.7M rows total). |
| `sql/schema/employer_directors_migration.sql` | NEW 2026-05-03 | Schema for `employer_directors` table, `load_def14a_progress` skip-tracker, and `director_interlocks` view (24Q-13). Applied 2026-05-03; awaits parser. |
...
| `scripts/maintenance/audit_union_explorer.py` | NEW 2026-05-05 / ACTIVE | Master orchestrator for the Union Explorer audit harness. Runs L1 (SQL invariants), L2 (API↔DB diff), optionally L4 (DeepSeek), L5 (anomaly set), L6 (sentinels) in dependency order. Writes `audit_runs/<DATE>/audit_summary.md` + `auto_problems.md`. Wired into RELEASE_CHECKLIST. Flags: `--include-llm`, `--quick`, `--skip-layer N`, `--llm-max-cost-usd`. |
| `scripts/maintenance/audit_union_layer1.py` + `audit_union_truth_queries.sql` | NEW 2026-05-05 / ACTIVE | Layer 1: 20 deterministic SQL invariants in parseable `@check:` blocks. ~5 sec runtime. Hard-gate checks: financial_trends_overcount_regression (Bug 1 guard), national_endpoint_dedup_matches_canonical_view (30M-bug-class), soc_excluded_from_canonical_view, ar_membership/ar_disbursements_total rpt_id orphans. Advisory: coverage metrics, staleness, anomaly counts. |
| `scripts/maintenance/audit_union_layer2.py` | NEW 2026-05-05 / ACTIVE | Layer 2: API↔DB consistency on ~270-union sample × 9 checks + 191 affiliations. Modes: `http` (default, hits :8001), `asgi` (in-process FastAPI TestClient), `direct`. Disables rate limiter via `RATE_LIMIT_REQUESTS=0` env var. 100% pass on 2026-05-05 first run. |
| `scripts/maintenance/audit_union_layer4.py` | NEW 2026-05-05 / ACTIVE | Layer 4: DeepSeek-V3 advisory rubric, async pool concurrency 8, JSON-mode strict output, reasoner escalation when V3 score ≤2 / members or assets >10× reference. Cost cap via `--max-cost-usd`. Reasoner JSON-parse currently fails 13/13 (thinking-content prefix; low-priority follow-up). |
| `scripts/maintenance/audit_union_layer5.py` + `audit_union_anomaly_frozen.yaml` | NEW 2026-05-05 / ACTIVE | Layer 5: 7 deterministic anomaly queries (large_active_zero_nlrb, active_no_recent_filing, members_jump_or_drop_50pct, employer_under_multiple_unions, etc), frozen suppressions YAML (13 federal/RLA aff_abbr whitelisted), diff vs prior run. |
| `scripts/maintenance/audit_union_layer6.py` | NEW 2026-05-05 / ACTIVE | Layer 6: 10 hand-picked union profiles validated against expected JSON shape across 4 endpoints. Substitute for Codex's Playwright recommendation (Playwright not in repo). 10/10 pass. |
| `scripts/maintenance/audit_union_reference_cards.yaml` | NEW 2026-05-05 / ACTIVE | 31 hand-curated affiliation reference cards: approx_active_members bounds, expected_industries, nlrb_jurisdiction, expected_election_volume, notes. Used by Layer 4 to keep DeepSeek from hallucinating Wikipedia. Covers NEA/AFT/SEIU/AFSCME/IBT/UFCW/UAW/IBEW/CWA/IAM/IUOE/LIUNA/CJA/IATSE/USW/AFGE/NALC/APWU/NTEU/ATU/SMART/UNITE-HERE/PPF/BCTD/IAFF/SAGAFTRA/NFOP/NNU/WGAW/WGAE/TWU. |
