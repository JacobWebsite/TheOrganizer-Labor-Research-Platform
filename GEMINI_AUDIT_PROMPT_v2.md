...
## ⚙️ SETUP: How to Connect to the Database

**Read this first before doing anything else.** The database is PostgreSQL running locally. The simplest way to connect is with a short Python script. Here's a working template:
...
If `psycopg2` isn't installed, run `pip install psycopg2-binary python-dotenv` first.

**If this approach fails**, read the `.env` file directly to get the credentials and try connecting manually. The important thing is that you **actually run queries against the live database** — don't just read documentation and assume it's accurate.

---

## 🔴 CRITICAL INSTRUCTION: You Must Actually Verify Data With Queries

This audit requires you to **run real SQL queries and report real numbers from the live database.** Reading documentation files and summarizing what they claim is NOT sufficient. The whole point of this audit is to check whether the documentation matches reality.

For every number you report (row counts, match rates, null percentages, orphan counts), you must:
1. Actually run the query
2. Show the query you ran
3. Show the result you got

If you find yourself writing things like "according to CLAUDE.md, there are 60,953 employers" — STOP. Instead, run `SELECT count(*) FROM f7_employers_deduped` and report what the database actually says.
...
**Connection approach:** Use the Python template above. If you prefer `psql`, that's fine too — but Python with `psycopg2` is the most reliable method for this environment.
...
Save your report as: `docs/AUDIT_REPORT_ROUND2_GEMINI.md`
...
4. **🔴 CRITICAL — Check for orphaned records:**

   This is extremely important. "Orphaned records" means data that references other data that no longer exists — like an address book entry for a building that's been demolished. The system still says "go here" but there's nothing there.

   Run ALL of these checks and report the exact counts:

   ```sql
   -- Orphan check 1: employer_ids in the union-employer link table that don't exist in the employer table
   SELECT count(*) 
   ...
   -- Orphan check 2: union file numbers in the link table that don't exist in the union table
   SELECT count(*) 
   FROM f7_union_employer_relations r
   LEFT JOIN unions_master u ON r.f_num = u.f_num
   WHERE u.f_num IS NULL;

   -- Orphan check 3: NLRB participants pointing to elections that don't exist
   SELECT count(*) 
   FROM nlrb_participants p
   LEFT JOIN nlrb_elections e ON p.case_number = e.case_number
   WHERE e.case_number IS NULL;

   -- Orphan check 4: OSHA matches pointing to employers that don't exist
   SELECT count(*) 
   FROM osha_f7_matches m
   LEFT JOIN f7_employers_deduped e ON m.employer_id = e.employer_id
   WHERE e.employer_id IS NULL;

   -- Orphan check 5: OSHA matches pointing to establishments that don't exist
   SELECT count(*) 
   FROM osha_f7_matches m
   LEFT JOIN osha_establishments o ON m.establishment_id = o.id
   WHERE o.id IS NULL;
   ```

   **Why this matters so much:** In Round 1 of this audit, approximately 50% of union-employer links were orphaned — meaning half the data connections in the system were broken. This was the single biggest finding. You MUST check this and report real numbers.

5. Report a summary of all orphan checks:

   ```
   | Relationship | Total Records | Orphaned | Orphan Rate |
   |-------------|--------------|----------|-------------|
   | f7_relations → employers | ? | ? | ?% |
   | f7_relations → unions | ? | ? | ?% |
   | nlrb_participants → elections | ? | ? | ?% |
   | osha_matches → employers | ? | ? | ?% |
   | osha_matches → establishments | ? | ? | ?% |
   ```
...
## SECTION 11 (BONUS): Strategic & Architectural Vision

This section is in addition to the standard audit — it asks you to think bigger about where this platform should go.

**11.1 — Data Source Gaps**
What important government or public data sources are NOT yet in this platform but should be? For each one:
- What the data source is and what it contains
- Why it matters for union organizing
- How hard it would be to integrate (API available? Bulk download? Scraping required?)
- How it would connect to existing data

Consider sources like: FMCS (contract expirations), EEO-1 (workforce demographics), state-level labor boards, MSHA (mine safety), FEC/OpenSecrets (political contributions), news monitoring, and others.

**11.2 — Matching Architecture**
The platform currently uses a tiered matching approach (exact match → normalized match → fuzzy match). Is this the right architecture? Consider:
- Would a probabilistic matching library (like Splink) be better as the primary approach?
- How should matching be standardized across all data sources?
- What about using employer identifiers (EIN, DUNS) as the primary join key instead of name matching?

**11.3 — Deployment Readiness**
If the goal is to let 5-10 union organizers use this platform via a web browser within 6 months, what are the top 5 things that need to happen? Think about: hosting, security, user management, data freshness, documentation for non-technical users.

**11.4 — Database Evolution**
The platform has no system for tracking database structure changes over time (like adding tables or columns). What should be implemented? Consider migration tools, version tracking, and reproducibility.

**11.5 — Predictive Analytics Potential**
Given the data that exists (NLRB election outcomes, OSHA violations, employer characteristics, industry patterns), what kinds of predictive models could realistically be built? What would be needed to train and validate them responsibly?
...
9. **Run the orphan checks.** Sections 2 has explicit SQL queries for orphan detection. Run every single one and report the numbers. Do not skip these.
10. **Verify with queries, not documentation.** Whenever you state a number, it must come from a query you ran — not from reading a docs file.
