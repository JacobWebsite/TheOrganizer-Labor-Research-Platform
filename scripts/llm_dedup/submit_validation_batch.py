...
BATCH_INPUT    = os.path.join(DIR, 'anthropic_validation_batch_input.jsonl')
MANIFEST_PATH  = os.path.join(DIR, 'anthropic_validation_batch_manifest.json')
STATE_PATH     = os.path.join(DIR, 'anthropic_validation_batch_state.json')
RESULTS_JSONL  = os.path.join(DIR, 'anthropic_validation_batch_results.jsonl')
RESULTS_CSV    = os.path.join(DIR, 'anthropic_validation_batch_results.csv')
...
def cmd_submit(args):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        last_err = None
        # Up to 6 attempts with exponential backoff (10s, 20s, 40s, 80s, 120s, 180s).
        # Anthropic seems to rate-limit rapid batch creations, so we give it
        # plenty of time to recover before aborting.
        for attempt in range(1, 7):
            ...
            try:
                ...
                last_err = e
                print(f'    attempt {attempt}/6 failed: {type(e).__name__}: {str(e)[:120]}')
                if attempt < 6:
            ...
            print(f'    ALL ATTEMPTS FAILED for chunk {i}. Aborting; resume with '
                  f'--skip-first {(args.skip_first or 0) + (i - 1) * chunk_size + len(chunk)}')
            return 1
        # Longer pause between chunks to avoid per-batch-creation rate limits.
        # Anthropic appears to throttle rapid batch creation; 30s works reliably.
        if i < len(chunks):
            print('    pausing 30s before next chunk...')
            time.sleep(30)
...
def cmd_fetch(args):
    ...
    with open(RESULTS_JSONL, 'w', encoding='utf-8') as out_jsonl, \
        ...
        for entry in all_entries:
            ...
            # Schema-tolerant: see parse_verdict_obj() docstring. Fixes the
            # silent UNKNOWN-everywhere bug from the 2026-04-21 batch where
            # the v2.0 validation prompt's 'label' key was missed.
