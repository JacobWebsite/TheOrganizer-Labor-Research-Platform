"""Run 14 methods (M1-M5 + M1b,M3b + M1c-M5d) on companies, report summaries + CSV.

Usage:
    py scripts/analysis/demographics_comparison/run_comparison_400_v3.py
    py scripts/analysis/demographics_comparison/run_comparison_400_v3.py --companies selected_holdout_v3.json

Requires selected_400.json or specified JSON.
Uses CachedLoadersV3 for all methods.
14 methods: 5 originals (M1-M5, no M6) + 2 V2 (M1b, M3b) + 7 V3 variants.
Note: M2b/M4b/M5b/M7 dropped -- replaced by their V3 successors.

Actually per plan: 5 surviving originals (M1, M2, M3, M3b, M1b) + 9 V3 = 14.
"""
import sys
import os
import json
...
from methodologies_v3 import OPTIMAL_WEIGHTS_V3_BY_GROUP, OPTIMAL_WEIGHTS_V3_BY_CATEGORY
...
# Build combined method registry: 14 methods
# 5 originals: M1, M2, M3, M4, M5 (no M6)
# 2 V2 survivors: M1b, M3b
# 9 V3 new: M1c, M1d, M2c, M3c, M3d, M4c, M4d, M5c, M5d
COMBINED_METHODS = {}
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run V3 demographics comparison')
    parser.add_argument('--companies', default='selected_400.json',
                        help='JSON file with selected companies (default: selected_400.json)')
...
def run_all_methods(cl, naics4, state_fips, county_fips, state_abbr='', zipcode=''):
    """Run all 14 methods using cached loaders.

    Methods that need extra kwargs (state_abbr, zipcode) get them.
    """
    results = {}
    for name, method_fn in COMBINED_METHODS.items():
        try:
            if name in V3_METHODS_NEED_EXTRA:
...
def print_dimension_tables(all_results):
    """Print 5 dimension summary tables (Race MAE) for 14 methods."""
...
def print_overall_summary(method_stats, n_companies):
    ...
    # Print M1c and M5c weights
    print('')
    print('CROSS-VALIDATED WEIGHTS')
    print('-' * 60)
    print('M1c weights (per NAICS group):')
    for group in sorted(OPTIMAL_WEIGHTS_V3_BY_GROUP.keys()):
        acs_w, lodes_w = OPTIMAL_WEIGHTS_V3_BY_GROUP[group]
        print('  %-40s ACS=%.2f LODES=%.2f' % (group, acs_w, lodes_w))
    print('')
    print('M5c weights (per M5 category):')
    for cat in sorted(OPTIMAL_WEIGHTS_V3_BY_CATEGORY.keys()):
        acs_w, lodes_w = OPTIMAL_WEIGHTS_V3_BY_CATEGORY[cat]
        print('  %-20s ACS=%.2f LODES=%.2f' % (cat, acs_w, lodes_w))
...
def main():
    ...
    if 'holdout_v3' in input_basename:
        tag = 'holdout_v3'
    ...
    print('DEMOGRAPHICS COMPARISON V3 (14 methods)')
    ...
    detail_path = os.path.join(SCRIPT_DIR, 'comparison_%s_v3_detailed.csv' % tag)
    summary_path = os.path.join(SCRIPT_DIR, 'comparison_%s_v3_summary.csv' % tag)
