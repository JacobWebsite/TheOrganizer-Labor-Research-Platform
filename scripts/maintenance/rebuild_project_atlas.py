...
SYNTHETIC_TASKS: list[dict] = [
    ...
    # --- 2026-05-10 / 2026-05-11 ship-branch closures (launch roadmap W4-W6) ---
    # The launch roadmap (ROADMAP_2026_05_04_to_2026_07_05_LAUNCH.md) isn't parsed
    # by ROADMAP_MD; ship branches are the source of truth for "what got done".
    {"id": "W4-sec-10k-stop-list-iter2", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "SEC 10-K extractor stop-list iter 2 (geo noise + acronyms + sentence-boundary)",
     "description": "Closed 2026-05-11 on ship/2026-05-10-sec-10k-stop-list (commit f4d451d). +21 entries to _STOP_EXACT (geographic noise like Los Angeles/New York/North American, acronyms IoT/LNG/OLED/PCBs, sentence-boundary 'no.'/'Our'). +2 regression tests with 6 overreach guards. Cumulative 1,491->1,326 entities, 34.4%->35.8% match, 38 spurious matches eliminated.",
     "status": "done", "effort": "30m", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-refresh-all-tail-check", "tier": "T0-R7", "section": "Process Debt (R7-Meta)",
     "title": "refresh_all.py invokes check_critical_mvs.py at end",
     "description": "Closed 2026-05-11 on ship/2026-05-11-refresh-all-tail-check (commit 28c55fe). Recurrence-prevention for the 'critical MVs vanish silently' bug that bit us 3x in 3 weeks. Added CRITICAL_MVS_SCRIPT constant + run_critical_mvs_check() helper invoked after print_summary(); non-zero rc -> ERROR + sys.exit(1). 4 monkeypatch tests at tests/test_refresh_all_tail_check.py.",
     "status": "done", "effort": "30m", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-a2-facilities-map", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "A.2 FacilitiesMap card on master profile",
     "description": "Closed 2026-05-11 on ship/2026-05-11-facilities-map (commit 792f1f5). 5/5 backend + 10/10 frontend audit verdict Ship-Ready. Spec deviation (EPA + F-7 + Mergent instead of OSHA + EPA + state-contracts) documented in card docstring. Live on Abbott showing 7 EPA facilities.",
     "status": "done", "effort": "frontend audit + ship", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-q15-competitors", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "Q15 Competitors card + endpoint",
     "description": "Closed 2026-05-11 on ship/2026-05-11-q16-19-rollup (commit 1b55032). New api/routers/competitors.py + CompetitorsCard.jsx + 11 tests.",
     "status": "done", "effort": "2-3h", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-q16-q17-q19-relationships", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "Q16/17/19 Suppliers/Customers/Distribution end-to-end chain",
     "description": "Closed 2026-05-11 on ship/2026-05-11-q16-19-rollup (commit 1b55032). sec_10k pipeline + relationships router + 4 cards (Suppliers/Customers/Distribution/RelationshipBody) mounted at EmployerProfilePage.jsx:273-279. Full-corpus extract: 1,491 entities -> 513 matched (34.4%). Live smoke on Fortinet (master 4245176): Suppliers '13 matched | 32 mentioned', Customers 1/1, Distribution 3/3.",
     "status": "done", "effort": "6-agent sprint", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-c5-power-profile-pdf", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "C.5 Power Profile PDF export",
     "description": "Closed 2026-05-11 on ship/2026-05-11-q16-19-rollup (commit 1b55032). 931 + 815 lines + 10 tests covering generation + auth + edge cases.",
     "status": "done", "effort": "1-2d", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-polish-7-cards", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "W4 A.3 polish sweep on 7 cards",
     "description": "Closed 2026-05-11 on ship/2026-05-11-q16-19-rollup (commit 1b55032). 7 employer-profile cards refined for empty/loading/error states + accessibility + density.",
     "status": "done", "effort": "ongoing", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-targets-perf-fix", "tier": "T0-R7", "section": "P0 R7-NEW Beta Blockers",
     "title": "Organizing Targets endpoint perf fix (42s -> 6s)",
     "description": "Closed 2026-05-11 on ship/2026-05-11-targets-perf-fix (commits e3c8b95+cd557a9+9f4472e). Bisected to LEFT JOIN (SELECT master_id, COUNT(DISTINCT source_system) FROM master_employer_source_ids GROUP BY master_id) -- PG can't push LIMIT through GROUP BY. Fix 1: LATERAL rewrite (42s -> 5s). Fix 2: partial composite index idx_master_employers_non_union_targets. Critical PG discovery: partial-predicate matcher is asymmetric -- COALESCE-form query won't match COALESCE-form partial predicate but WILL match OR-form. Also needed ANALYZE after CREATE INDEX. Codex /wrapup crosscheck flagged 2 BUGs (ts.source_count undercounts by 1-3 + has_lda/has_epa_echo missing; hardcoded master_id in partial index), both fixed in 9f4472e.",
     "status": "done", "effort": "investigation + 3 commits", "source": "ship branch", "closure_date": "2026-05-11"},
    {"id": "W4-q16-19-rollup-audit-tools", "tier": "T0-R7", "section": "Process Debt (R7-Meta)",
     "title": "audit_boardcard_quality.py + tier_validation_post_p0_5.py audit scripts",
     "description": "Closed 2026-05-11 on ship/2026-05-11-q16-19-rollup (commit 1b55032). audit_boardcard_quality.py (1,121 lines) + tier_validation_post_p0_5.py (439 lines) + wiring. Part of 5/9 sprint rollup -- 49 files / 11,277 insertions; 125/125 backend tests pass.",
     "status": "done", "effort": "1d", "source": "ship branch", "closure_date": "2026-05-11"},

    # --- 2026-05-12 ship-branch closures (5-agent parallel sweep) ---
    {"id": "W5-mv-target-data-sources-undercount-fix", "tier": "T0-R7", "section": "P0 R7-NEW Beta Blockers",
     "title": "mv_target_data_sources.source_count undercount + F7-only master exclusion",
     "description": "Closed 2026-05-12 on ship/2026-05-12-target-data-sources-fix (commit 14d255c). Filed 2026-05-11 by Codex /wrapup crosscheck on the targets perf branch. SUM at build_target_data_sources.py:106-117 was missing has_f7/has_lda/has_epa_echo/has_fec; downstream WHERE tds.source_count>=1 was excluding F7-only masters entirely. Expanded source_flags CTE + SELECT + SUM. Rebuilt mv_target_data_sources (5,790,584 rows, 145s) + mv_target_scorecard (5,783,197 rows, +381,146 vs pre-fix, 595s). 75,378 F7-only masters now in scorecard. Source count distribution 1->14 (was capped at 12). Reapplied cd557a9-style precomputed-ts.source_count optimization in master.py (targets perf branch can now safely use the MV). 6 new regression tests + 71/71 scoring + 43/43 API.",
     "status": "done", "effort": "11min MV rebuild + tests", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-frontend-no-data-empty-states", "tier": "T1", "section": "W4 24Q Coverage Sprint",
     "title": "Frontend No-Data Cards inconsistent empty states + DataFreshness test verified",
     "description": "Closed 2026-05-12 on ship/2026-05-12-frontend-no-data-polish (commit 81ce16a). New shared EmptyStateCard component (amber AlertTriangle panel + canonical 'No <topic> records have been matched...' copy + optional reason ReactNode). 6 cards migrated (ComparablesCard, CorporateHierarchyCard, DataProvenanceCard, CrossReferencesSection, UnionRelationshipsCard, WorkforceDemographicsCard no-usable-data branch). 4 self-gating cards intentionally NOT migrated (DirectorNetworkSection/FamilyRollupSection/ResearchInsightsCard/BasicProfileView). DataFreshness test was already passing on baseline -- 2026-03-18 fix held. 379/379 frontend tests (+10 new EmptyStateCard tests).",
     "status": "done", "effort": "audit + 6 card migrations", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-future-date-cleanup-fec-sec-xbrl", "tier": "T0-R7", "section": "P0 R7-NEW Beta Blockers",
     "title": "FEC + SEC XBRL garbage future dates cleanup + 5 CHECK constraints + silent NameError fix",
     "description": "Closed 2026-05-12 on ship/2026-05-12-future-date-cleanup (commit d850d54). DISCOVERY: 2026-05-03 FEC fix had a silent NameError -- `from datetime import datetime, timezone` was missing `date`, but _norm_date called date.today(). Every parseable FEC date raised NameError, defeating the year-range guard entirely. Cleaned 72 garbage rows: fec_individual_contributions 1 (>today+1y, NULL'd), fec_candidates 46 (cand_election_yr >today+10, NULL'd), sec_xbrl_financials 25 (filed_date<fiscal_year_end, DELETE'd). 5 CHECK constraints added: chk_fec_indiv/pas2_transaction_dt_sane, chk_fec_candidates_election_yr_sane, chk_sec_xbrl_fy_end_sane, chk_sec_xbrl_filed_after_fy_end -- also embedded in CREATE_TABLE_SQL so DROP+CREATE preserves. 25-case regression test (helper sanitization + DB invariants + CHECK presence + live INSERT 3312-01-01 rejection).",
     "status": "done", "effort": "investigation + cleanup + constraints", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-small-ops-sweep-may-12", "tier": "T2", "section": "Process Debt (R7-Meta)",
     "title": "Small-ops sweep: Vite proxy + dead code + state_local_contractor placeholder + CLAUDE.md counts",
     "description": "Closed 2026-05-12 on ship/2026-05-12-small-ops-sweep (commit c04568e). DISCOVERY: stale state_local_contractor column refs in build_employer_data_sources.py were ACTUALLY BROKEN (SQL referenced non-existent columns -- 'column does not exist' errors, not just dead refs). Fixed with NULL::boolean/0::integer placeholder strategy + 10-line comment. CLAUDE.md count drift: backend 1,482->1,590, frontend 344->379 (49 files), crosswalk 39,827->38,794. 3 CLAUDE.md files updated with 2026-05-12 verification stamps. Vite proxy already at canonical :8001 (verified). UnifiedScorecardPage page+tests already deleted (verified). 236 scoring tests + 379/379 frontend still green.",
     "status": "done", "effort": "30m", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-parser-bug-and-fnum-investigation", "tier": "T2", "section": "Process Debt (R7-Meta)",
     "title": "Validation CSV verdict parser hardening + 138 unresolved f_nums investigation",
     "description": "Closed 2026-05-12 on ship/2026-05-12-parser-fix-and-union-investigation (commit 8e7cffc). Parser bug: 2026-05-02 inline fix in submit_anthropic_batch.py + submit_validation_batch.py was untested. Extracted parse_verdict_obj() to module-level helper + isinstance(dict) guard for non-dict JSON. 14 regression tests passing. Investigation: 138 unresolved f_nums STILL exactly 138 (no drift from 2026-03-19 baseline). Pulled all 35,268 OLMS public filer records via POST /olpdr/GetFilerListServlet (endpoint 2026-03-19 declared 'no auto resolution path' -- actually exists). ALL 138/138 are in OLMS with terminated:'T', termDates 2000-2011 (peak 2008-2009). Recommendation: backfill into unions_master as is_likely_inactive=true entries with termDate (~half day, low blast radius, post-launch P2).",
     "status": "done", "effort": "30m + investigation", "source": "ship branch", "closure_date": "2026-05-12"},

    # --- 2026-05-12 ROUND 2 ship-branch closures (6-agent parallel sweep, PM session) ---
    {"id": "W5-matching-quality-fixes", "tier": "T1", "section": "1A. Matching Quality",
     "title": "10-K matcher suffix-stripping asymmetry + fuzzy band FP token-overlap gate",
     "description": "Closed 2026-05-12 on ship/2026-05-12-matching-quality-fixes (commit 2e80823). DISCOVERY: master_employers.canonical_name only lowercases + replaces non-alphanumerics; doesn't strip Inc/LLC/Corp. 10-K matcher comparing normalize_name_aggressive(entity_text) against canonical_name silently missed 'Walmart Inc' vs 'walmart inc'. Built upstream fix: new canonical_name_aggressive column + GIN trgm + partial equality indexes; new aggressive_form_of_canonical() Python helper. Matcher swap is queued for whenever ship/2026-05-11-q16-19-rollup merges (documented in note). Expected match-rate lift ~32% -> ~50%. Fuzzy FP gate: token-overlap rule (score>=0.90 bypass; <0.90 require overlap>=0.40) with 3 rescue paths (collapsed-spacing equality, prefix-extension bounded 1.5x, edit-distance>=0.90 with length-ratio>=0.85). All 5 flagship FP cases from the Open Problem note (Walmart vs Wal-Mart Pharmacy / Acme vs Apex / GM vs GE / Apple vs Apple Hospital / Bank of America vs America First) correctly rejected. Production-rate gate trigger: ~0.2% per F7-to-F7 TABLESAMPLE. 32 new tests + 176/176 matching tests pass.",
     "status": "done", "effort": "5233s of agent work", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-cba-rule-tuning", "tier": "T2", "section": "CBA Provision Extraction",
     "title": "CBA Rule Coverage Gaps + Config Possibly Relocated re-verification",
     "description": "Closed 2026-05-12 on ship/2026-05-12-cba-rule-tuning (commit d2937ec). Config relocation: false alarm. rule_engine.py:15 path resolves fine; 14/14 categories load; 109 provision classes / regex patterns all compile. Coverage Gaps: state had improved dramatically since the note (141 CBAs / 5,302 provisions; 98/109 = 89.9% classes match). 11 zero-match classes (probationary_period, vesting, retirement_eligibility, pension_fund, overtime_after, schedule_posting, seniority_bidding, ai_provisions, apprentice_ratio, training_time_paid, union_access) all closed with 13 new patterns across 7 JSON files. 17 new tests; 172/172 CBA tests pass. Also: refined parallel-agent branch-race recovery technique (use `git update-ref` instead of `reset --hard` to avoid re-flipping working tree).",
     "status": "done", "effort": "1112s of agent work", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-scoring-pillar-fixes", "tier": "T1", "section": "1C. Scoring Quality",
     "title": "Stability Pillar coverage + Scoring Weights predictive-power design memo",
     "description": "Closed 2026-05-12 on ship/2026-05-12-scoring-pillar-fixes (commit 5ede572). Both Open Problems already RESOLVED 2026-04-03 (D12 / D13 scoring redesign). Stability Pillar: irreducible at F7 layer (only 3,693 / 146,863 F7 employers have a form5500 link in master_employer_source_ids; bottleneck is the F7<->Form 5500 crosswalk, not the SQL). Added 2 regression tests locking the 4 Form 5500 passthrough columns into MV schema + coverage floor. Scoring Weights: produced design memo (docs/scratch/scoring_weights_predictive_power_2026_05_12.md, 144 lines) recommending Option C -- defer to post-launch when real outcome data accumulates. 78/78 scoring tests pass.",
     "status": "done", "effort": "579s of agent work", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-research-agent-phases", "tier": "T1", "section": "1B. Research Agent Value",
     "title": "Research Agent enhancement wiring gap + manual review workflow",
     "description": "Closed 2026-05-12 on ship/2026-05-12-research-agent-phases (commit adda69e). DISCOVERY: compute_research_enhancements() was only called from batch_research.py, not from single-run agent.py path. 140 completed runs were eligible for enhancement, but only 59 enhancements existed -- 58% miss rate. Fixed wiring at agent.py:~2207-2234 (call after grade_and_save, use grade_result return value to avoid redundant DB SELECT, try/except so failed enhancement doesn't fail the run). Plus: new list_dossiers_needing_review.py (310 lines, 5 priority modes, CSV/TSV output, deep links) + RESEARCH_REVIEW_WORKFLOW.md (127 lines, weekly cadence 5 dossiers x 5 priority facts = 25 reviews/week). 26 new tests pass.",
     "status": "done", "effort": "1233s of agent work", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-freshness-and-gower", "tier": "T2", "section": "Process Debt (R7-Meta)",
     "title": "data_source_freshness broken date_query refs + Gower pipeline re-verify",
     "description": "Closed 2026-05-12 on ship/2026-05-12-freshness-and-gower (commit 5b5349d). DISCOVERY: 2026-05-04 freshness sweep validated count_query only, missed 2 broken date_query refs: (1) lm_filings referenced fiscal_year_start/fiscal_year_end (don't exist) -- fixed to make_date(MIN(yr_covered),1,1) ... MAX(yr_covered); (2) match_tables referenced matched_at (doesn't exist) -- fixed to created_at::date. Added --validate-only mode to create_data_freshness.py + 3 regression tests. Gower pipeline re-verified: 18.3M employer_comparables rows + 80.43% score_similarity coverage = healthy. NEW BUG SURFACED: compute_gower_similarity.py --dry-run is DESTRUCTIVE -- create_table() always DROPs employer_comparables regardless of dry-run, then repopulates only 1 NAICS block (~120K rows). Smoke test wiped 18.3M -> 124K. Required restoration: 4hr compute_gower_similarity.py --skip-view full re-run (kicked off post-T9). MV regression flagged for 4th time -- needs ship/2026-05-11-refresh-all-tail-check merged.",
     "status": "done", "effort": "1563s of agent work", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-backfill-138-terminated-fnums", "tier": "T2", "section": "OLMS Union Reference",
     "title": "Backfill 138 terminated f_nums into unions_master as is_likely_inactive entries",
     "description": "Closed 2026-05-12 on ship/2026-05-12-backfill-138-terminated-fnums (commit 0b03326). Implements T5 recommendation from morning sweep. Added term_date DATE column to unions_master (is_likely_inactive already existed). Loaded 138 rows from OLMS public-filer JSON (gathered 2026-05-12 AM). Result: f7_union_employer_relations orphan rows 344 -> 0; distinct orphan f_nums 138 -> 0; unions_master 26,693 -> 26,831 (+138). Spot-checks confirm OLMS data (540479=ANA LU 296 / 1458=Teamsters LU 478 / 24058=UAW LU 153). 5 new tests + 35/35 union tests pass.",
     "status": "done", "effort": "699s of agent work", "source": "ship branch", "closure_date": "2026-05-12"},
    {"id": "W5-mv-restoration-may-12-pm", "tier": "T0-R7", "section": "P0 R7-NEW Beta Blockers",
     "title": "MV restoration 4th recurrence (refresh_all --skip-gower) + Gower table 4hr restore",
     "description": "Closed 2026-05-12 PM (operational, no commit). Hit 4th recurrence of 'critical MVs missing' (Open Problem from 2026-05-08). Ran refresh_all.py --skip-gower (646s, 5 MVs restored: mv_target_scorecard / mv_employer_search / mv_unified_scorecard / mv_target_data_sources / mv_employer_features). First attempt failed on master because build_employer_data_sources is_state_local_contractor placeholder fix is on unmerged ship/2026-05-12-small-ops-sweep (c04568e) -- switched to that branch to complete. Important merge-order finding: small-ops-sweep is a prerequisite for any other branch that needs refresh_all to work. Also kicked off compute_gower_similarity.py --skip-view (~4hr) to restore employer_comparables from 124K back to 18.3M after T5's accidental wipe.",
     "status": "done", "effort": "operational", "source": "session ops", "closure_date": "2026-05-12"},
    # --- 2026-07-11/12 integration catch-up + demo session (see memory/session_2026_07_11_integration_beta_dossier_expansion.md) ---
    {"id": "W7-ship-branch-backlog-drained", "tier": "T0-R8", "section": "Launch Sprint W7-W9",
     "title": "Ship-branch backlog fully drained + origin un-frozen (78 commits pushed)",
     "description": "Closed 2026-07-11. Merged the final 6 ship branches (June-7 launch-blocks R8-5 identity-grafting-gate + R8-7 search-ranking-perf 1313ms->86ms; T5 docs rollup subsuming both wrapup-docs; drop-director-overfetch deferral note). Verified via merge-base loop: ZERO unmerged ship/* remain. Pushed origin (was frozen at May-8 86ca1ed, 78 commits behind). Full suite on merged master 2,196 passed / 8 skipped / 0 failed. Closed vault Open Problems: Unmerged Ship-Branch Backlog, mv_target_data_sources source_count undercount, Identity Grafting Crouse->Children's National. 9 stale worktrees removed.",
     "status": "done", "effort": "1 session", "source": "session ops", "closure_date": "2026-07-11"},
    {"id": "W8-beta-onboarding-flow", "tier": "T0-R8", "section": "Launch Sprint W7-W9",
     "title": "Beta tester onboarding flow (welcome page + feedback capture)",
     "description": "Closed 2026-07-11 on ship/2026-07-11-beta-onboarding (26d9feb, merge 890a74f). /welcome page (intro + how-to-read-a-dossier walkthrough + CTAs), persistent floating Feedback button -> FeedbackModal -> POST /api/feedback -> NEW feedback_submissions table; Getting Started nav link; /api/feedback added to critical_routes (gate 20/20). 4 backend + 7 frontend tests; browser-verified E2E. Hardened 2026-07-12 per Codex crosscheck (ship/2026-07-12-feedback-hardening, 1f48ee7): GET /api/feedback admin-gated (fails closed); button tags employer_id from /employers/:id routes.",
     "status": "done", "effort": "~7h est, 1 session actual", "source": "roadmap W8", "closure_date": "2026-07-11"},
    {"id": "W6-dossier-expansion-100", "tier": "T1", "section": "Launch Sprint W7-W9",
     "title": "Dossier expansion 38->100 (beta-state batch) + --states filter",
     "description": "Closed 2026-07-12 on ship/2026-07-12-dossier-expansion (merge 9cdbaa5). Added --states to batch_research.py (both candidate queries; Codex-cleared for injection). Ran 61-run NY/OH/VA batch: 57 ok + 4 Gemini-flake retries all ok, avg quality 6.05; +Yale (#299, 6.27) +Montefiore (#300, 6.61). research_runs: 255 completed / 74 gold (33 QC-flagged at >=6.0 per Jacob's rule; 24 sub-6.0 await his spot-check). Approved list: docs/scratch/dossier_expansion_61_list_2026_07_11.md (NY38/VA12/OH11). Total API spend < $5.",
     "status": "done", "effort": "~6h wall (background)", "source": "roadmap W6/W9", "closure_date": "2026-07-12"},
    {"id": "W4-flagship-five-picked", "tier": "T1", "section": "Launch Sprint W7-W9",
     "title": "Flagship gold-standard demo set picked + wall-to-wall verified",
     "description": "Closed 2026-07-12 (vault Decision 2026-07-12 - Flagship Five Demo Set and Gold Flagging). Set: Montefiore (9/9 signals, Anger 9.7), Yale (NLRB 10.0: 12 elections/67 ULP, $5.39M lobbying, 14,257 FEC donations), Graceland Fruit (small-shop, 27 OSHA violations), Pacific Lutheran + Bronson (backups). Starbucks/Lowe's/Dollar Tree REJECTED: retail mega-chain masters are data-thin (1 source each) -- universities/hospitals are the platform's richest recognizables. All 5 profiles browser-verified. 3 demo warts logged as Open Problems (No-Known-Union on unionized flagships; Yale 59-workers header; nonprofit 'Public company' label).",
     "status": "done", "effort": "1 session", "source": "roadmap W4 (slipped)", "closure_date": "2026-07-12"},
]
