...
FIXED: Handle copyright footer rows
"""
...
def load_state_density():
    ...
    for filepath in sorted(state_files):
        ...
        year_str = filename.replace('state_', '').replace('.xlsx', '').replace(' (1)', '').strip()
        try:
            year = int(year_str)
        except ValueError:
        ...
        try:
            # Read Excel file, skip header rows (row 0 is title, row 1 is empty, row 2 is headers)
            ...
            # Filter out rows where state is null or state_census_code is not a valid number
            # This removes copyright footers and empty rows
            df = df[df['state'].notna()]
            df = df[df['state_census_code'].apply(is_valid_number)]
            
            # Skip if no valid data
            if len(df) == 0:
                print(f"  {year}: No valid data rows")
                continue
            ...
            for _, row in df.iterrows():
                data.append((
                    year,
                    int(float(row['state_census_code'])),
                    str(row['state']),
                    str(row['sector']),
...
def load_industry_density():
    ...
    for filepath in INDUSTRY_FILES:
        ...
        filename = os.path.basename(filepath)
        year_str = filename.replace('ind_', '').replace('.xlsx', '').replace(' (1)', '').strip()
        try:
            year = int(year_str)
        except ValueError:
            print(f"  Skipping {filename} - can't parse year")
        ...
        try:
            ...
            # Filter out rows where industry is null or contains copyright
            df = df[df['industry'].notna()]
            df = df[~df['industry'].astype(str).str.contains('Copyright', case=False, na=False)]
            ...
            for _, row in df.iterrows():
                ...
                cic = None
                if pd.notna(row['cic_code']) and is_valid_number(row['cic_code']):
                    cic = int(float(row['cic_code']))
                
                data.append((
                    year,
                    cic,
                    str(row['industry']),
                    int(row['observations']) if pd.notna(row['observations']) and is_valid_number(row['observations']) else None,
                    float(row['employment_thousands']) if pd.notna(row['employment_thousands']) else None,
                    float(row['members_thousands']) if pd.notna(row['members_thousands']) else None,
                    float(row['covered_thousands']) if pd.notna(row['covered_thousands']) else None,
                    float(row['pct_members']) if pd.notna(row['pct_members']) else None,
...
def verify_load():
    ...
    # Sample data
    print("\n  Sample state data (NY 2023):")
    pg_cursor.execute("""
        SELECT state, sector, ROUND(pct_members::numeric * 100, 1) as pct_union
        FROM unionstats_state 
        WHERE year = 2023 AND state = 'New York'
    """)
...
if __name__ == "__main__":
    print("="*60)
    print("UNIONSTATS DATA LOADER (FIXED)")
    print("="*60)
