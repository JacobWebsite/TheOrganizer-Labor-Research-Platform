...
Usage:
    py scripts/etl/geocode_batch_run.py                # dry-run (default)
    py scripts/etl/geocode_batch_run.py --apply        # apply to DB
    py scripts/etl/geocode_batch_run.py --po-box-only  # only process PO Box fallback
"""
...
def process_po_boxes(cur, apply_mode, centroids):
    ...
    if not os.path.exists(po_box_file):
        ...
        print("  Run geocode_batch_prep.py first.")
        return 0, 0
