...
def backfill(conn, dry_run=False):
    ...
    with conn.cursor() as cur:
        ...
        if dry_run:
            # Show sample
            for r in rows[:5]:
                eid, name = r[0] if isinstance(r, (list, tuple)) else r['employer_id'], \
                            r[1] if isinstance(r, (list, tuple)) else r['employer_name']
                print(f"  {str(name)[:50]:50s} -> std={normalize_name_standard(name)[:30]}, "
        ...
        updates = []
        for r in rows:
            eid = r[0] if isinstance(r, (list, tuple)) else r['employer_id']
            name = r[1] if isinstance(r, (list, tuple)) else r['employer_name']
            updates.append((
