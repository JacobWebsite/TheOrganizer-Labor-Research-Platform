"""
Labor Relations Platform API v4.2 - Enhanced with Fuzzy & Normalized Search
...
app = FastAPI(
    ...
    version="4.2",
    description="Employer search with union density, projections, fuzzy matching, and name normalization"
)
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT 
                    ...
                    LEFT(e.naics, 2) as naics_2digit, ns.sector_name as naics_sector_name,
                    e.latest_unit_size, e.latest_union_fnum, e.latest_union_name,
                    e.latest_notice_date, e.latitude, e.longitude,
                    e.healthcare_related, e.potentially_defunct,
                    um.aff_abbr, um.union_name as union_display_name,
                    um.members as union_members, um.sector as union_sector
                FROM f7_employers_deduped e
                LEFT JOIN unions_master um ON e.latest_union_fnum::text = um.f_num
                LEFT JOIN naics_sectors ns ON LEFT(e.naics, 2) = ns.naics_2digit
                WHERE {where_clause}
            ...
            return {"total": total, "limit": limit, "offset": offset, "employers": cur.fetchall()}


# ============================================================================
# BLS UNION DENSITY & PROJECTIONS
# ============================================================================
...
def get_all_density(year: Optional[int] = None):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            if year:
                cur.execute("""
                    SELECT bnm.naics_2digit, ns.sector_name, bnm.bls_indy_name, bud.year, bud.value as union_density_pct
...
def get_naics_projections(naics_2digit: str, limit: int = Query(20, le=100)):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT naics_code, industry_title, employment_2024, employment_2034, employment_change, employment_change_pct
                FROM bls_industry_projections
                WHERE naics_code = %s OR naics_code LIKE %s
                ORDER BY CASE WHEN naics_code = %s THEN 0 ELSE 1 END, ABS(employment_change) DESC
                LIMIT %s
            """, [naics_2digit, f"{naics_2digit}%", naics_2digit, limit])
            projections = cur.fetchall()
            
            cur.execute("""
                SELECT SUM(employment_2024) as total_2024, SUM(employment_2034) as total_2034,
                       SUM(employment_change) as total_change, AVG(employment_change_pct) as avg_employment_change_pct
                FROM bls_industry_projections WHERE naics_code LIKE %s
            """, [f"{naics_2digit}%"])
...
def get_top_projections(growing: bool = Query(True), limit: int = Query(20, le=100)):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT LEFT(naics_code, 2) as naics_2digit, ns.sector_name, naics_code, industry_title,
                       employment_2024, employment_2034, employment_change, employment_change_pct
                ...
                ORDER BY employment_change_pct {order} LIMIT %s
            """, [limit])
...
# ============================================================================
# FUZZY NAME SEARCH - Phase 1 (pg_trgm)
# ============================================================================
...
def fuzzy_search_employers(
    ...
    threshold: float = Query(0.3, ge=0.1, le=1.0, description="Similarity threshold"),
...
def find_similar_employers(
    ...
    same_state: bool = Query(True),
    limit: int = Query(20, le=100)
):
    """Find employers similar to a given employer (for deduplication)."""
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT employer_name, city, state FROM f7_employers_deduped WHERE employer_id = %s", [employer_id])
            source = cur.fetchone()
            if not source:
            ...
            cur.execute(f"""
                SELECT e.employer_id, e.employer_name, similarity(e.employer_name, %s) as match_score,
                       e.city, e.state, e.latest_unit_size, e.latest_union_name, um.aff_abbr
                FROM f7_employers_deduped e
                LEFT JOIN unions_master um ON e.latest_union_fnum::text = um.f_num
                WHERE e.employer_id != %s AND similarity(e.employer_name, %s) > %s {state_filter}
                ORDER BY similarity(e.employer_name, %s) DESC LIMIT %s
            """, params)
            
            return {"source_employer": source, "threshold": threshold, "same_state_only": same_state, "similar_employers": cur.fetchall()}
...
# ============================================================================
# NORMALIZED SEARCH - Phase 2 (Name Normalization)
# ============================================================================
...
def normalized_search_employers(
    ...
    """
    Search with NAME NORMALIZATION + fuzzy matching.
    
    Removes: "The", "Inc", "LLC", "Corp", "Company", etc.
    So "The Kroger Company" and "Kroger" both match.
    """
    # Normalize search term using Python function
    normalized_search = normalize_employer(name, strip_location=True)
    
    with get_db() as conn:
        with conn.cursor() as cur:
            state_filter = "AND state = %s" if state else ""
            
            # Use SQL normalization to match
            norm_sql = """
                LOWER(REGEXP_REPLACE(
                    REGEXP_REPLACE(employer_name, '^[Tt]he\\s+', ''),
                    '\\s*(Inc\\.?|LLC|Corp\\.?|Company|Co\\.?|Ltd\\.?)\\s*$', '', 'gi'
                ))
            """
            ...
            cur.execute(f"""
                ...
                WHERE similarity({norm_sql}, %s) > %s {state_filter}
            """, count_params)
            total = cur.fetchone()['total']
            ...
            cur.execute(f"""
                SELECT employer_id, employer_name, {norm_sql} as normalized_name,
                       similarity({norm_sql}, %s) as match_score,
                       city, state, naics, latest_unit_size, latest_union_fnum, latest_union_name,
                       latitude, longitude
                FROM f7_employers_deduped
                WHERE similarity({norm_sql}, %s) > %s {state_filter}
                ORDER BY similarity({norm_sql}, %s) DESC, latest_unit_size DESC NULLS LAST
                LIMIT %s OFFSET %s
            """, select_params)
...
def normalized_search_employers(
    ...
    """
    NORMALIZED + FUZZY search for employers.
    
    Normalizes both search term AND database names before comparing:
    - Strips: Inc, LLC, Corp, Company, Co, Ltd, etc.
    - Expands: Hosp->Hospital, Med->Medical, Ctr->Center
    - Removes: The, A, An, Of, And
    
    So "The Kroger Company" matches "Kroger" with score 1.0!
    """
    # Normalize search term (aggressive: expand abbrevs + remove stopwords)
    from name_normalizer import normalize_for_comparison
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            # SQL-based normalization (simplified - strips common suffixes)
            norm_sql = """
                LOWER(
                    TRIM(
                        REGEXP_REPLACE(
                            REGEXP_REPLACE(
                                REGEXP_REPLACE(
                                    REGEXP_REPLACE(employer_name, 
                                        '^[Tt]he\\s+', ''),
                                    ',?\\s*(Inc\\.?|LLC|Corp\\.?|Company|Co\\.?|Ltd\\.?|LP|LLP|PC|PA)\\s*
, '', 'gi'),
                                '\\s+', ' ', 'g'),
                            '^\\s+|\\s+
, '', 'g')
...
def normalized_search_unions(
    ...
    Normalizes union names:
    - Expands: Int'l->International, Brthd->Brotherhood, etc.
    - Preserves acronyms: SEIU, UFCW, IBT, etc.
    - Normalizes local numbers: "Local #1000" -> "local 1000"
    """
    from name_normalizer import normalize_union
    normalized_search = normalize_union(name)
    
    with get_db() as conn:
        with conn.cursor() as cur:
            # Simple SQL normalization for unions
            norm_sql = "LOWER(REGEXP_REPLACE(union_name, '\\s+', ' ', 'g'))"
            ...
            cur.execute(f"""
                ...
                ORDER BY similarity({norm_sql}, %s) DESC, members DESC NULLS LAST
                LIMIT %s
...
def test_normalization(
    name: str = Query(...),
    entity_type: str = Query("employer", description="employer or union")
):
    """
    Test the normalization functions without searching.
