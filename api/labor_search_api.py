"""
Labor Relations Search API - Updated for deduplicated data and local numbers
Run with: py -m uvicorn labor_search_api:app --reload --port 8000
"""
...
app = FastAPI(
    ...
    description="Search 71,000+ employers and union locals from DOL filings (2020-2025)",
    version="2.0"
)
...
# Database configuration - uses environment variable or default
import os
PG_CONFIG = {
    'host': 'localhost',
    'database': 'olms_multiyear',
    'user': 'postgres',
    'password': 'Juniordog33!'  # Set PGPASSWORD env var or edit this
}
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE local_count > 0 OR f7_employer_count > 0
                ORDER BY f7_employer_count DESC NULLS LAST
            """)
            return cur.fetchall()
...
def search_employers(
    ...
    """Search employers with flexible filters"""
    conditions = []
    params = []
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT employer_id, employer_name, city, state, 
                       ...
                       union_file_number, union_name_f7, affiliation, affiliation_name
...
def search_locals(
    ...
    """Search union locals with flexible filters"""
    conditions = []
    params = []
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT file_number, union_name, local_display_name, local_number,
                       affiliation, affiliation_name, city, state, members,
                       total_assets, total_receipts, f7_employer_count, f7_total_workers
...
def get_local_detail(file_number: str):
    """Get detailed union local info including financial history and employers"""
    with get_db() as conn:
        with conn.cursor() as cur:
            # Current year data
            cur.execute("""
                SELECT file_number, union_name, local_display_name, local_number,
                       affiliation, affiliation_name, desig_name, city, state, members,
            ...
            cur.execute("""
                SELECT rpt_id, f_num as file_number, union_name, aff_abbr as affiliation,
                       desig_name, desig_num as local_number, city, state,
                       yr_covered as fiscal_year, members,
            ...
            # Employers from deduplicated F-7 table
            cur.execute("""
                SELECT employer_id, employer_name, city, state, 
...
def get_local_employers(file_number: str, limit: int = 100, offset: int = 0):
...
@app.get("/states")
def list_states():
    """List states with union density and F-7 employer counts"""
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT 
                    ...
                    u.pct_members as union_density,
                    u.members_thousands * 1000 as union_members,
                    COALESCE(f7.employer_count, 0) as f7_employers,
                    COALESCE(f7.union_count, 0) as unions_with_employers
                FROM state_abbrev s
                LEFT JOIN unionstats_state u ON s.state_name = u.state 
                    AND u.year = 2024 AND u.sector = 'Total'
                LEFT JOIN (
...
def get_state_employers(
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(f"""
                SELECT employer_id, employer_name, city, state, 
                       bargaining_unit_size, union_file_number, union_name_f7,
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                employer_count = cur.fetchone()['count']
                cur.execute("SELECT COUNT(*) FROM lm_data WHERE yr_covered = 2024")
                union_count = cur.fetchone()['count']
        return {
            "status": "healthy",
            "database": "connected",
            "f7_employers": employer_count,
            "union_locals_2024": union_count
        }
